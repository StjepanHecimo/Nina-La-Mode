...
export async function sendOrderEmails(order: EmailOrder) {
  ...
  const from = { name: process.env.SMTP_FROM_NAME || "Nina La Mode", address: process.env.SMTP_FROM_EMAIL! };
  ...
  await Promise.all([
    transporter.sendMail({
      ...
      replyTo: shopEmail,
      subject: `We received your Nina La Mode order ${order.orderNumber}`,
