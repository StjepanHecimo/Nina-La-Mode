...
type SubmitAction = "save" | "draft" | "publish";
...
export function AdminProductForm({ email, initialProduct }: { email: string; initialProduct?: AdminProduct }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const submitAction: SubmitAction = submitter?.value === "publish" ? "publish" : submitter?.value === "save" ? "save" : "draft";
    if (!existingImages.length && !files.length) { setError("Add at least one product image."); return; }
  ...
  return <>
    ...
    <form className="admin-product-form" onSubmit={submit}>
      ...
      <section><h2>Product specifications</h2><div className="field-row"><label>Colour name<input name="colourName" defaultValue={primaryColour?.name ?? initialProduct?.color} placeholder="Deep Green" required /></label><label>Colour swatch<input name="colourHex" type="color" defaultValue={primaryColour?.hex ?? "#173f48"} required /></label></div><label>Material<input name="material" defaultValue={initialProduct?.material} placeholder="100% cotton" required /></label><div className="field-row"><label>Inseam<input name="inseam" defaultValue={initialProduct?.inseam} placeholder="88 cm" /></label><label>Fit<input name="fit" defaultValue={initialProduct?.fit} placeholder="Classic high waist with a wide-leg silhouette" /></label></div><fieldset className="admin-sizes"><legend>Available UK sizes</legend>{ukSizes.map(size => <label key={size}><input name="sizes" type="checkbox" value={size} defaultChecked={initialProduct?.sizes.includes(size)} />{size}</label>)}</fieldset></section>
      <section className="admin-images-section"><h2>Product images</h2><p>The first image is the main image. Remove unwanted images or add new JPG, PNG or WebP files up to 6 MB each.</p>{existingImages.length > 0 && <div className="admin-image-previews">{existingImages.map((url, index) => <figure key={url}><Image src={url} alt="" width={150} height={190} /><figcaption>{index === 0 ? "Main image" : `Image ${index + 1}`}</figcaption><div>{index > 0 && <button type="button" onClick={() => setExistingImages(current => [url, ...current.filter(item => item !== url)])}>Make main</button>}<button type="button" className="danger" onClick={() => setExistingImages(current => current.filter(item => item !== url))}>Remove</button></div></figure>)}</div>}<label className="admin-file-picker">Add images<input type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={(event) => setFiles(Array.from(event.target.files || []))} /></label>{previews.length > 0 && <div className="admin-image-previews new-images">{previews.map(preview => <figure key={preview.url}><Image src={preview.url} alt="" width={150} height={190} unoptimized /><figcaption>{preview.name}</figcaption></figure>)}</div>}</section>
      <section className="admin-publish"><h2>{editing ? "Save changes" : "Save or publish"}</h2><p>{editing && initialProduct?.active ? "Save updates without sending another newsletter." : <><strong>Save draft</strong> keeps the product private. <strong>Publish and notify subscribers</strong> makes it visible and immediately sends one Brevo new-arrival campaign.</>}</p>{error && <p className="checkout-error" role="alert">{error}</p>}{message && <p className="admin-success" role="status">{message}</p>}<div>{editing && initialProduct?.active ? <button className="button" type="submit" value="save" disabled={Boolean(action)}>{action === "save" ? "Saving…" : "Save changes"}</button> : <><button className="button secondary" type="submit" value={editing ? "save" : "draft"} disabled={Boolean(action)}>{action === "save" || action === "draft" ? "Saving…" : editing ? "Save draft changes" : "Save draft"}</button><button className="button" type="submit" value="publish" disabled={Boolean(action)}>{action === "publish" ? "Publishing…" : "Publish and notify subscribers"}</button></>}</div></section>
    </form>
  </>;
}
