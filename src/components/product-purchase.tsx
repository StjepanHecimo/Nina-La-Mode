...
export function ProductPurchase({ product }: { product: Product }) {
  ...
  function addToBag() {
    if (!size) return;
    addItem({ productId: product.id, name: product.name, image: product.image, size, colour, quantity: 1, priceCents: product.priceCents, preorder: product.availability === "coming_soon" });
    setAdded(true);
  }
  ...
  return <>
    ...
    {product.availability === "coming_soon" && <p className="preorder-note"><strong>Available for pre-order.</strong> We will confirm the expected delivery date by email.</p>}
    <button className="button detail-cta" type="button" disabled={!size} onClick={addToBag}>{added ? "Added to bag" : product.availability === "coming_soon" ? "Pre-order" : "Add to bag"} <ArrowIcon /></button>
